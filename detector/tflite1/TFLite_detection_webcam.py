...
class VideoStream:
    ...
    def __init__(self, resolution=(640, 480), framerate=30):
        ...
        #ret = self.stream.set(5,cv2.cv.CV_FOURCC(*'XVID'))
...
while True:
    ...
    for i in range(len(scores)):
		...
		print("nothing of interest")
            ...
            cv2.rectangle(
                frame, (xmin, label_ymin-labelSize[1]-10), (xmin+labelSize[0], label_ymin+baseLine-10), (255, 255, 255), -1)
            cv2.putText(frame, label, (xmin, label_ymin-7),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)  # Draw label text

    # Draw framerate in corner of frame
    cv2.putText(frame, 'FPS: {0:.2f}'.format(
        frame_rate_calc), (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2, cv2.CV_AA)
